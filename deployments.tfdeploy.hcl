...
orchestrate "auto_approve" "safe_plans" {
  ...
  check { condition = context.plan.changes.remove == 0 reason = "Plan has ${context.plan.changes.remove} resources to be destroyed." }

  # Ensure that the deployment is not production or disaster-recovery
  check { condition = context.plan.deployment.name != "production" && context.plan.deployment.name != "disaster-recovery" reason = "Production and Disaster Recovery plans are not eligible for auto_approve." } 
}
